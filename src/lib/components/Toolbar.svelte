<script>
  ...
  // The state variable for the restored checkbox
  ...
  async function handlePlatformExport(platform) {
    if (packageCompound) {
      ...
      // Visual confirmation that the checkbox state was recognized
      alert(
      ...
      try {
        ...
        // Inject a README file to force OS auto-extractors to create a folder rather than leaving a single JSON file
        zip.file(
          "README_Export.txt",
        ...
        for (let i = 0; i < catalog.length; i++) {
          ...
          // Prefix with the floor index to keep them chronologically organized and prevent name collisions
      ...
      {#if displayCategory === "select"}
        <div class="panel-section">
          ...
          <label class="checkbox-row" style="margin-bottom: 8px;">
            <input
              type="checkbox"
              checked={packageCompound}
              onchange={(e) => (packageCompound = e.target.checked)}
            />
